...
“显示格式”可选择阅读视图、JSON 格式（两空格缩进）或原文。JSON 格式默认用占位说明折叠图片 Base64，可勾选“显示完整图片数据”；非 JSON 正文自动回到原文并提示。复制始终使用完整脱敏原文。

检测结果每个模型的每次尝试右上角提供报文图标。弹窗可切换请求／响应，阅读视图按字段展开、保留提示词换行并解析响应 content 内的 JSON 字符串，图片 Data URL 默认折叠且可完整展开；原文视图与“复制完整正文”保留脱敏后的完整正文。HTTP 方法、实际 URL、状态码及脱敏报文头单独展示。

新调用从 HTTPX 发送钩子捕获 SDK 最终序列化的请求，收到响应后保留正文与状态；不是根据参数重建。捕获请求说明已进入发送流程，不代表供应商已成功收到。请求和响应在尝试结束时保存，响应超时仍可保存请求。旧记录没有完整请求时明确提示缺失，不追溯伪造。密钥、Authorization、Cookie 及凭据类报文头脱敏；请求正文含完整图片数据，因此会增加本地数据库体积。

## 按事件直接切片检测

在事件列表每行右侧点击分割图标按钮（描边为关闭，绿色实底为开启），无需打开提示词编辑区（默认关闭，也可用于垃圾以外的事件）。该设置与完整 JSON 的事件字段 `tile_detection: true` 同步；未勾选事件不会进入本轮，恢复默认会关闭切片，复用历史会加载该轮设置。切片开关按事件编码及“只看已选事件”状态保存在当前浏览器，刷新后恢复；规则文本不保存。全选和清空选择只改变参与检测状态，不改变切片设置。

沿用 hos-analysis 的几何规则：处理后图片宽高比 ≥ 3 时，横向切成 3 张全高图片，相邻重叠约 25%；非宽图仍用整图。开启事件直接使用切片，不做整图初筛或二次复核。其他事件使用整图；全部事件开启时只发送三张切片。每个图片与模型组合仍只调用一次，提示词按事件分配视图，要求模型逐片独立判断、边界证据不足不命中、多片命中取最高置信度并注明视图编号。输出保持原有 events 协议，服务端不额外校验逐片判断过程。

轮次冻结每张图片的视图坐标、事件分配及实际提示词；不同模型及重试使用相同输入。结果区“实际提示词”和尝试详情可核对，费用继续按单次响应的真实 usage 计算；多图输入可能增加图像用量。真实模型的多图能力与识别效果仍需实际 API 验收。

## 结果区切换图片

检测结果上方提供吸顶图片导航，显示当前序号和文件名，可通过上一张、下一张或横向缩略图直接切换。切换后定位到对应结果起点，无需返回上传区；首尾按钮自动禁用。点击“放大原图”就地查看当前原始图片，关闭或按 Escape 后保留阅读位置。历史轮次沿用相同导航。

原图入口位于缩略图上：仅鼠标悬浮时显示右上角放大图标，点击只预览该图片，不切换当前结果。鼠标移开即隐藏；点击图片本身仍切换结果。
阅读视图会直接预览报文内 PNG/JPEG/WebP/GIF 的 Base64 Data URL 图片，完整编码继续折叠保留；无法解码时提示查看原文。
