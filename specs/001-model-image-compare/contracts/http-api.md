...
GET /api/attempts/{id} 对新调用提供可选 request_body（SDK 最终发送 JSON 文本）、request_http（method/url/脱敏 headers）及 response_http（status_code/脱敏 headers）；raw_response 保持脱敏响应正文。尚未结束、尚未发送及旧记录可缺少这些字段，不拼接替代。轮次摘要不返回 request_body；报文弹窗按尝试 ID 延迟读取详情，不触发模型请求。

prompt_text 的每项事件支持可选 `tile_detection: boolean`，缺省 false，非布尔值拒绝整轮。开启后，轮次详情增加 `image_detection_inputs`（逐图 views/version/prompt），尝试详情及摘要增加 `detection_input`。宽图按事件分配整图或三切片，在同一次请求中直接检测；非宽图回到整图。页面实际提示词优先展示当前图片冻结的 prompt，历史对比保留逐图输入；重试沿用冻结输入。
...
事件切片开关按事件编码、“只看已选事件”按布尔状态保存到当前站点 localStorage，刷新恢复，包括关闭、未选事件的切片及空选择过滤。直接编辑 JSON 的切片字段同样保存，规则文本不保存；恢复默认清除切片并关闭过滤，复用历史以该轮设置覆盖当前偏好。存储损坏时使用默认状态。

模型结果卡片的报文图标与标题垂直居中；弹窗提供“阅读视图／JSON 格式／原文”选择。JSON 格式按两空格缩进展示，默认以占位说明折叠图片 Base64，可勾选显示完整图片数据，非 JSON 正文保留原文并提示；切换格式不改变存储或复制内容。

工作台左侧图片组与预览，右侧模型、四项参数、完整提示词。下方按当前图片展示各模型卡片，折叠原文和实际参数。每秒刷新活动轮次摘要，终态停止轮询。刷新页面后可从历史恢复活动轮次。

图片和模型参数在上方并排；下方事件工作区占整行，左侧紧凑目录、右侧当前事件编辑器，支持搜索和只看已选。运行按钮正常排在工作区下方，不浮动遮挡内容。手机端目录和编辑器上下排列。

事件列表每行右侧直接提供无文字的分割图标切换按钮，悬停显示状态和操作提示，aria-pressed 表达开关状态，无需进入事件提示词编辑区，点击不切换当前编辑事件。全选及清空选择仅改变参与检测状态，不改变切片设置；恢复默认关闭全部切片开关。

提示词按 events 独立展示，支持名称/编码搜索、全选、清空选择及切换编辑事件字段；完整 JSON 在折叠区与单项编辑同步。POST /api/rounds 的 prompt_text 仅包含已选 events，其余顶层字段保留；未选事件仅保留于当前页面草稿，不进入历史。空选择或无效 JSON 在页面阻止提交，服务端仍执行原有完整校验。默认全选；复用历史加载该轮已保存事件并全选。
